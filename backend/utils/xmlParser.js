const xml2js = require('xml2js');

// convert string to number
function toNumber(v) {
  if (v == null || v === '') return 0;
  const n = Number(String(v).replace(/[^0-9.\-]/g, ''));
  return Number.isNaN(n) ? 0 : n;
}

// convert string to array
function ensureArray(v) {
  if (v == null) return [];
  return Array.isArray(v) ? v : [v];
}

async function parseXml(xmlString) {
  const parser = new xml2js.Parser({ explicitArray: false, mergeAttrs: true, trim: true });
  const parsedXML = await parser.parseStringPromise(xmlString);


  // normalize
  const root = parsedXML?.INProfileResponse ?? parsedXML;



//   Basic details:
  const currApp = root?.Current_Application?.Current_Application_Details;
  const applicant = currApp?.Current_Applicant_Details;
  const firstName = applicant?.First_Name ?? applicant?.FirstName ?? null;
  const middleName1 = applicant?.Middle_Name1 ?? null;
  const middleName2 = applicant?.Middle_Name2 ?? null;
  const middleName3 = applicant?.Middle_Name3 ?? null;
  const middleName = middleName1+middleName2+middleName3;
  const lastName = applicant?.Last_Name ?? applicant?.LastName ?? null;
  const mobile = applicant?.MobilePhoneNumber ?? applicant?.Telephone_Number_Applicant_1st ?? null;


//   let pan = root?.CAIS_Account?.CAIS_Account_DETAILS?.CAIS_Holder_ID_Details?.Income_TAX_PAN ?? null;
  let pan = null;
  const idNodes = ensureArray(root?.CAIS_Account?.CAIS_Account_DETAILS)
    .flatMap(n => ensureArray(n?.CAIS_Holder_ID_Details ?? []));
  for (const idn of idNodes) {
    if (idn?.Income_TAX_PAN) { pan = idn.Income_TAX_PAN; break; }
  }
  if (!pan) {
    const holder = root?.CAIS_Account?.CAIS_Account_DETAILS?.CAIS_Holder_Details;
    pan = holder?.Income_TAX_PAN ?? null;
  }

//   console.log(pan);

  // Score
  const score = toNumber(root?.SCORE?.BureauScore ?? root?.SCORE?.Score);

  // Report summary (CAIS_Summary)
  const caisSummary = root?.CAIS_Account?.CAIS_Summary;
  const creditAccount = caisSummary?.Credit_Account ?? {};
  const totalAccounts = toNumber(creditAccount?.CreditAccountTotal);
  const activeAccounts = toNumber(creditAccount?.CreditAccountActive);
  const closedAccounts = toNumber(creditAccount?.CreditAccountClosed);

  const totOut = caisSummary?.Total_Outstanding_Balance ?? {};
  const currentBalanceAmount = toNumber(totOut?.Outstanding_Balance_All);
  const securedAmount = toNumber(totOut?.Outstanding_Balance_Secured);
  const unsecuredAmount = toNumber(totOut?.Outstanding_Balance_UnSecured);

  const last7Enquiries = toNumber(root?.TotalCAPS_Summary?.TotalCAPSLast7Days) || 0;

// Credit Accounts Information:
  const accNodes = ensureArray(root?.CAIS_Account?.CAIS_Account_DETAILS);
  const accounts = accNodes.map(acc => {
    const bank = (acc?.Subscriber_Name || '').trim() || null;
    const accountNumber = acc?.Account_Number ?? acc?.AccountNo ?? null;
    const portfolioType = acc?.Portfolio_Type ?? null; 
    const accountTypeCode = acc?.Account_Type ?? null;
    const currentBalance = toNumber(acc?.Current_Balance);
    const amountOverdue = toNumber(acc?.Amount_Past_Due ?? acc?.Amount_Past_Due);


    // Card Holder address:
    const holderAddr = acc?.CAIS_Holder_Address_Details;
    let address = null;
    if (holderAddr) {
      const parts = [
        holderAddr?.First_Line_Of_Address_non_normalized,
        holderAddr?.Second_Line_Of_Address_non_normalized,
        holderAddr?.Third_Line_Of_Address_non_normalized,
        holderAddr?.City_non_normalized,
        holderAddr?.ZIP_Postal_Code_non_normalized,
        holderAddr?.State_non_normalized
      ].filter(Boolean);
      address = parts.length ? parts.join(', ') : null;
    }
    const isCreditCard = (portfolioType === 'R') || ['10','51','52'].includes(String(accountTypeCode));

    return {
      bank,
      accountNumber,
      portfolioType,
      accountTypeCode,
      isCreditCard,
      amountOverdue,
      currentBalance,
      address
    };
  });

  return {
    basicDetails: {
      name: [firstName, middleName, lastName].filter(Boolean).join(' ') || null,
      mobilePhone: mobile || null,
      pan: pan || null,
      creditScore: score || null
    },
    reportSummary: {
      totalAccounts,
      activeAccounts,
      closedAccounts,
      currentBalanceAmount,
      securedAccountsAmount: securedAmount,
      unsecuredAccountsAmount: unsecuredAmount,
      last7DaysEnquiries: last7Enquiries
    },
    creditAccountsInfo: accounts.map(a => ({
        bank: a.bank,
        address: a.address,
        accountNumber: a.accountNumber,
        amountOverdue: a.amountOverdue,
        currentBalance: a.currentBalance,
      })),
  };
}

// const xml = `
// <INProfileResponse>
// <Header>
// <SystemCode>0</SystemCode>
// <MessageText/>
// <ReportDate>20200723</ReportDate>
// <ReportTime>171559</ReportTime>
// </Header>
// <UserMessage>
// <UserMessageText>Normal Response</UserMessageText>
// </UserMessage>
// <CreditProfileHeader>
// <Enquiry_Username>batchusr_matchtest_noinquiry_ecv_custom_match</Enquiry_Username>
// <ReportDate>20200723</ReportDate>
// <ReportTime>171559</ReportTime>
// <Version>V2.4</Version>
// <ReportNumber>1595504758919</ReportNumber>
// <Subscriber/>
// <Subscriber_Name>Bureau Disclosure Report with Customized Match</Subscriber_Name>
// </CreditProfileHeader>
// <Current_Application>
// <Current_Application_Details>
// <Enquiry_Reason>6</Enquiry_Reason>
// <Finance_Purpose/>
// <Amount_Financed>0</Amount_Financed>
// <Duration_Of_Agreement>0</Duration_Of_Agreement>
// <Current_Applicant_Details>
// <Last_Name>ugle</Last_Name>
// <First_Name>Sagar</First_Name>
// <Middle_Name1/>
// <Middle_Name2/>
// <Middle_Name3/>
// <Gender_Code/>
// <IncomeTaxPan/>
// <PAN_Issue_Date/>
// <PAN_Expiration_Date/>
// <Passport_number/>
// <Passport_Issue_Date/>
// <Passport_Expiration_Date/>
// <Voter_s_Identity_Card/>
// <Voter_ID_Issue_Date/>
// <Voter_ID_Expiration_Date/>
// <Driver_License_Number/>
// <Driver_License_Issue_Date/>
// <Driver_License_Expiration_Date/>
// <Ration_Card_Number/>
// <Ration_Card_Issue_Date/>
// <Ration_Card_Expiration_Date/>
// <Universal_ID_Number/>
// <Universal_ID_Issue_Date/>
// <Universal_ID_Expiration_Date/>
// <Date_Of_Birth_Applicant>00010201</Date_Of_Birth_Applicant>
// <Telephone_Number_Applicant_1st/>
// <Telephone_Extension/>
// <Telephone_Type/>
// <MobilePhoneNumber>9819137672</MobilePhoneNumber>
// <EMailId/>
// </Current_Applicant_Details>
// <Current_Other_Details>
// <Income>0</Income>
// <Marital_Status/>
// <Employment_Status/>
// <Time_with_Employer/>
// <Number_of_Major_Credit_Card_Held/>
// </Current_Other_Details>
// <Current_Applicant_Address_Details>
// <FlatNoPlotNoHouseNo/>
// <BldgNoSocietyName/>
// <RoadNoNameAreaLocality/>
// <City/>
// <Landmark/>
// <State/>
// <PINCode/>
// <Country_Code>IB</Country_Code>
// </Current_Applicant_Address_Details>
// <Current_Applicant_Additional_AddressDetails/>
// </Current_Application_Details>
// </Current_Application>
// <CAIS_Account>
// <CAIS_Summary>
// <Credit_Account>
// <CreditAccountTotal>4</CreditAccountTotal>
// <CreditAccountActive>3</CreditAccountActive>
// <CreditAccountDefault>0</CreditAccountDefault>
// <CreditAccountClosed>1</CreditAccountClosed>
// <CADSuitFiledCurrentBalance>0</CADSuitFiledCurrentBalance>
// </Credit_Account>
// <Total_Outstanding_Balance>
// <Outstanding_Balance_Secured>85000</Outstanding_Balance_Secured>
// <Outstanding_Balance_Secured_Percentage>35</Outstanding_Balance_Secured_Percentage>
// <Outstanding_Balance_UnSecured>160000</Outstanding_Balance_UnSecured>
// <Outstanding_Balance_UnSecured_Percentage>65</Outstanding_Balance_UnSecured_Percentage>
// <Outstanding_Balance_All>245000</Outstanding_Balance_All>
// </Total_Outstanding_Balance>
// </CAIS_Summary>
// <CAIS_Account_DETAILS>
// <Identification_Number/>
// <Subscriber_Name> icicibank</Subscriber_Name>
// <Account_Number>ICIVB20994</Account_Number>
// <Portfolio_Type>R</Portfolio_Type>
// <Account_Type>10</Account_Type>
// <Open_Date>19981213</Open_Date>
// <Credit_Limit_Amount>50000</Credit_Limit_Amount>
// <Highest_Credit_or_Original_Loan_Amount/>
// <Terms_Duration/>
// <Terms_Frequency/>
// <Scheduled_Monthly_Payment_Amount/>
// <Account_Status>11</Account_Status>
// <Payment_Rating>0</Payment_Rating>
// <Payment_History_Profile>N</Payment_History_Profile>
// <Special_Comment/>
// <Current_Balance>80000</Current_Balance>
// <Amount_Past_Due>4000</Amount_Past_Due>
// <Original_Charge_Off_Amount/>
// <Date_Reported>20180920</Date_Reported>
// <Date_of_First_Delinquency/>
// <Date_Closed/>
// <Date_of_Last_Payment/>
// <SuitFiledWillfulDefaultWrittenOffStatus/>
// <SuitFiled_WilfulDefault/>
// <Written_off_Settled_Status/>
// <Value_of_Credits_Last_Month/>
// <Occupation_Code/>
// <Settlement_Amount/>
// <Value_of_Collateral/>
// <Type_of_Collateral/>
// <Written_Off_Amt_Total/>
// <Written_Off_Amt_Principal/>
// <Rate_of_Interest/>
// <Repayment_Tenure>0</Repayment_Tenure>
// <Promotional_Rate_Flag/>
// <Income/>
// <Income_Indicator/>
// <Income_Frequency_Indicator/>
// <DefaultStatusDate/>
// <LitigationStatusDate/>
// <WriteOffStatusDate/>
// <DateOfAddition>20180920</DateOfAddition>
// <CurrencyCode>INR</CurrencyCode>
// <Subscriber_comments/>
// <Consumer_comments/>
// <AccountHoldertypeCode>1</AccountHoldertypeCode>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>09</Month>
// <Days_Past_Due>20</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Holder_Details>
// <Surname_Non_Normalized>SAGAR</Surname_Non_Normalized>
// <First_Name_Non_Normalized>UGLE</First_Name_Non_Normalized>
// <Middle_Name_1_Non_Normalized/>
// <Middle_Name_2_Non_Normalized/>
// <Middle_Name_3_Non_Normalized/>
// <Alias/>
// <Gender_Code>1</Gender_Code>
// <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
// <Passport_Number/>
// <Voter_ID_Number/>
// <Date_of_birth>19820322</Date_of_birth>
// </CAIS_Holder_Details>
// <CAIS_Holder_Address_Details>
// <First_Line_Of_Address_non_normalized>ANANDI VIHAR</First_Line_Of_Address_non_normalized>
// <Second_Line_Of_Address_non_normalized>DEHU ROAD</Second_Line_Of_Address_non_normalized>
// <Third_Line_Of_Address_non_normalized/>
// <City_non_normalized>PUNE</City_non_normalized>
// <Fifth_Line_Of_Address_non_normalized/>
// <State_non_normalized>27</State_non_normalized>
// <ZIP_Postal_Code_non_normalized>411047</ZIP_Postal_Code_non_normalized>
// <CountryCode_non_normalized>IB</CountryCode_non_normalized>
// <Address_indicator_non_normalized/>
// <Residence_code_non_normalized/>
// </CAIS_Holder_Address_Details>
// <CAIS_Holder_Phone_Details>
// <Telephone_Number>9819137672</Telephone_Number>
// <Telephone_Type/>
// <Telephone_Extension/>
// <Mobile_Telephone_Number/>
// <FaxNumber/>
// <EMailId/>
// </CAIS_Holder_Phone_Details>
// <CAIS_Holder_ID_Details>
// <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
// <PAN_Issue_Date/>
// <PAN_Expiration_Date/>
// <Passport_Number/>
// <Passport_Issue_Date/>
// <Passport_Expiration_Date/>
// <Voter_ID_Number/>
// <Voter_ID_Issue_Date/>
// <Voter_ID_Expiration_Date/>
// <Driver_License_Number/>
// <Driver_License_Issue_Date/>
// <Driver_License_Expiration_Date/>
// <Ration_Card_Number/>
// <Ration_Card_Issue_Date/>
// <Ration_Card_Expiration_Date/>
// <Universal_ID_Number/>
// <Universal_ID_Issue_Date/>
// <Universal_ID_Expiration_Date/>
// <EMailId/>
// </CAIS_Holder_ID_Details>
// <CAIS_Holder_ID_Details>
// <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
// <PAN_Issue_Date/>
// <PAN_Expiration_Date/>
// <Passport_Number/>
// <Passport_Issue_Date/>
// <Passport_Expiration_Date/>
// <Voter_ID_Number/>
// <Voter_ID_Issue_Date/>
// <Voter_ID_Expiration_Date/>
// <Driver_License_Number/>
// <Driver_License_Issue_Date/>
// <Driver_License_Expiration_Date/>
// <Ration_Card_Number/>
// <Ration_Card_Issue_Date/>
// <Ration_Card_Expiration_Date/>
// <Universal_ID_Number/>
// <Universal_ID_Issue_Date/>
// <Universal_ID_Expiration_Date/>
// <EMailId/>
// </CAIS_Holder_ID_Details>
// </CAIS_Account_DETAILS>
// <CAIS_Account_DETAILS>
// <Identification_Number/>
// <Subscriber_Name> icicibank</Subscriber_Name>
// <Account_Number>ICICI8131308</Account_Number>
// <Portfolio_Type>I</Portfolio_Type>
// <Account_Type>52</Account_Type>
// <Open_Date>19940718</Open_Date>
// <Credit_Limit_Amount/>
// <Highest_Credit_or_Original_Loan_Amount>50000</Highest_Credit_or_Original_Loan_Amount>
// <Terms_Duration/>
// <Terms_Frequency/>
// <Scheduled_Monthly_Payment_Amount/>
// <Account_Status>13</Account_Status>
// <Payment_Rating>0</Payment_Rating>
// <Payment_History_Profile>11110000000000001???????????????????</Payment_History_Profile>
// <Special_Comment/>
// <Current_Balance>0</Current_Balance>
// <Amount_Past_Due>0</Amount_Past_Due>
// <Original_Charge_Off_Amount/>
// <Date_Reported>20190130</Date_Reported>
// <Date_of_First_Delinquency/>
// <Date_Closed>20190110</Date_Closed>
// <Date_of_Last_Payment/>
// <SuitFiledWillfulDefaultWrittenOffStatus/>
// <SuitFiled_WilfulDefault/>
// <Written_off_Settled_Status/>
// <Value_of_Credits_Last_Month/>
// <Occupation_Code/>
// <Settlement_Amount/>
// <Value_of_Collateral/>
// <Type_of_Collateral/>
// <Written_Off_Amt_Total/>
// <Written_Off_Amt_Principal/>
// <Rate_of_Interest/>
// <Repayment_Tenure>0</Repayment_Tenure>
// <Promotional_Rate_Flag/>
// <Income/>
// <Income_Indicator/>
// <Income_Frequency_Indicator/>
// <DefaultStatusDate/>
// <LitigationStatusDate/>
// <WriteOffStatusDate/>
// <DateOfAddition>20170830</DateOfAddition>
// <CurrencyCode>INR</CurrencyCode>
// <Subscriber_comments/>
// <Consumer_comments/>
// <AccountHoldertypeCode>1</AccountHoldertypeCode>
// <CAIS_Account_History>
// <Year>2019</Year>
// <Month>01</Month>
// <Days_Past_Due>0</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>12</Month>
// <Days_Past_Due>40</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>11</Month>
// <Days_Past_Due>38</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>10</Month>
// <Days_Past_Due>36</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>09</Month>
// <Days_Past_Due>34</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>08</Month>
// <Days_Past_Due>25</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>07</Month>
// <Days_Past_Due>24</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>06</Month>
// <Days_Past_Due>23</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>05</Month>
// <Days_Past_Due>22</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>04</Month>
// <Days_Past_Due>21</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>03</Month>
// <Days_Past_Due>20</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>02</Month>
// <Days_Past_Due>19</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>01</Month>
// <Days_Past_Due>18</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>12</Month>
// <Days_Past_Due>17</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>11</Month>
// <Days_Past_Due>16</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>10</Month>
// <Days_Past_Due>15</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>09</Month>
// <Days_Past_Due>14</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>08</Month>
// <Days_Past_Due>45</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Holder_Details>
// <Surname_Non_Normalized>SAGAR</Surname_Non_Normalized>
// <First_Name_Non_Normalized>UGLE</First_Name_Non_Normalized>
// <Middle_Name_1_Non_Normalized/>
// <Middle_Name_2_Non_Normalized/>
// <Middle_Name_3_Non_Normalized/>
// <Alias/>
// <Gender_Code>1</Gender_Code>
// <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
// <Passport_Number/>
// <Voter_ID_Number/>
// <Date_of_birth>19820322</Date_of_birth>
// </CAIS_Holder_Details>
// <CAIS_Holder_Address_Details>
// <First_Line_Of_Address_non_normalized>J-1 110</First_Line_Of_Address_non_normalized>
// <Second_Line_Of_Address_non_normalized>ANANDI VIHAR</Second_Line_Of_Address_non_normalized>
// <Third_Line_Of_Address_non_normalized>DEHU ROAD</Third_Line_Of_Address_non_normalized>
// <City_non_normalized>DIGHI</City_non_normalized>
// <Fifth_Line_Of_Address_non_normalized/>
// <State_non_normalized>27</State_non_normalized>
// <ZIP_Postal_Code_non_normalized>412216</ZIP_Postal_Code_non_normalized>
// <CountryCode_non_normalized>IB</CountryCode_non_normalized>
// <Address_indicator_non_normalized/>
// <Residence_code_non_normalized/>
// </CAIS_Holder_Address_Details>
// <CAIS_Holder_Phone_Details>
// <Telephone_Number>9819137672</Telephone_Number>
// <Telephone_Type/>
// <Telephone_Extension/>
// <Mobile_Telephone_Number/>
// <FaxNumber/>
// <EMailId/>
// </CAIS_Holder_Phone_Details>
// <CAIS_Holder_ID_Details>
// <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
// <PAN_Issue_Date/>
// <PAN_Expiration_Date/>
// <Passport_Number/>
// <Passport_Issue_Date/>
// <Passport_Expiration_Date/>
// <Voter_ID_Number/>
// <Voter_ID_Issue_Date/>
// <Voter_ID_Expiration_Date/>
// <Driver_License_Number/>
// <Driver_License_Issue_Date/>
// <Driver_License_Expiration_Date/>
// <Ration_Card_Number/>
// <Ration_Card_Issue_Date/>
// <Ration_Card_Expiration_Date/>
// <Universal_ID_Number/>
// <Universal_ID_Issue_Date/>
// <Universal_ID_Expiration_Date/>
// <EMailId/>
// </CAIS_Holder_ID_Details>
// </CAIS_Account_DETAILS>
// <CAIS_Account_DETAILS>
// <Identification_Number/>
// <Subscriber_Name> icicibank</Subscriber_Name>
// <Account_Number>ICICI8131306</Account_Number>
// <Portfolio_Type>R</Portfolio_Type>
// <Account_Type>10</Account_Type>
// <Open_Date>19940716</Open_Date>
// <Credit_Limit_Amount>50000</Credit_Limit_Amount>
// <Highest_Credit_or_Original_Loan_Amount>50000</Highest_Credit_or_Original_Loan_Amount>
// <Terms_Duration/>
// <Terms_Frequency/>
// <Scheduled_Monthly_Payment_Amount/>
// <Account_Status>53</Account_Status>
// <Payment_Rating>1</Payment_Rating>
// <Payment_History_Profile>11110000000000001???????????????????</Payment_History_Profile>
// <Special_Comment/>
// <Current_Balance>80000</Current_Balance>
// <Amount_Past_Due>1100</Amount_Past_Due>
// <Original_Charge_Off_Amount/>
// <Date_Reported>20190130</Date_Reported>
// <Date_of_First_Delinquency/>
// <Date_Closed/>
// <Date_of_Last_Payment/>
// <SuitFiledWillfulDefaultWrittenOffStatus/>
// <SuitFiled_WilfulDefault>01</SuitFiled_WilfulDefault>
// <Written_off_Settled_Status>00</Written_off_Settled_Status>
// <Value_of_Credits_Last_Month/>
// <Occupation_Code/>
// <Settlement_Amount/>
// <Value_of_Collateral/>
// <Type_of_Collateral/>
// <Written_Off_Amt_Total/>
// <Written_Off_Amt_Principal/>
// <Rate_of_Interest/>
// <Repayment_Tenure>0</Repayment_Tenure>
// <Promotional_Rate_Flag/>
// <Income/>
// <Income_Indicator/>
// <Income_Frequency_Indicator/>
// <DefaultStatusDate/>
// <LitigationStatusDate/>
// <WriteOffStatusDate/>
// <DateOfAddition>20170830</DateOfAddition>
// <CurrencyCode>INR</CurrencyCode>
// <Subscriber_comments/>
// <Consumer_comments/>
// <AccountHoldertypeCode>1</AccountHoldertypeCode>
// <CAIS_Account_History>
// <Year>2019</Year>
// <Month>01</Month>
// <Days_Past_Due>36</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>12</Month>
// <Days_Past_Due>36</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>11</Month>
// <Days_Past_Due>34</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>10</Month>
// <Days_Past_Due>32</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>09</Month>
// <Days_Past_Due>30</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>08</Month>
// <Days_Past_Due>23</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>07</Month>
// <Days_Past_Due>22</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>06</Month>
// <Days_Past_Due>21</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>05</Month>
// <Days_Past_Due>20</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>04</Month>
// <Days_Past_Due>19</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>03</Month>
// <Days_Past_Due>18</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>02</Month>
// <Days_Past_Due>17</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>01</Month>
// <Days_Past_Due>16</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>12</Month>
// <Days_Past_Due>15</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>11</Month>
// <Days_Past_Due>14</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>10</Month>
// <Days_Past_Due>13</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>09</Month>
// <Days_Past_Due>12</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>08</Month>
// <Days_Past_Due>35</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Holder_Details>
// <Surname_Non_Normalized>SAGAR</Surname_Non_Normalized>
// <First_Name_Non_Normalized>UGLE</First_Name_Non_Normalized>
// <Middle_Name_1_Non_Normalized/>
// <Middle_Name_2_Non_Normalized/>
// <Middle_Name_3_Non_Normalized/>
// <Alias/>
// <Gender_Code>1</Gender_Code>
// <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
// <Passport_Number/>
// <Voter_ID_Number/>
// <Date_of_birth>19820322</Date_of_birth>
// </CAIS_Holder_Details>
// <CAIS_Holder_Address_Details>
// <First_Line_Of_Address_non_normalized>J-1 110</First_Line_Of_Address_non_normalized>
// <Second_Line_Of_Address_non_normalized>ANANDI VIHAR</Second_Line_Of_Address_non_normalized>
// <Third_Line_Of_Address_non_normalized>DEHU ROAD</Third_Line_Of_Address_non_normalized>
// <City_non_normalized>DIGHI</City_non_normalized>
// <Fifth_Line_Of_Address_non_normalized/>
// <State_non_normalized>27</State_non_normalized>
// <ZIP_Postal_Code_non_normalized>412216</ZIP_Postal_Code_non_normalized>
// <CountryCode_non_normalized>IB</CountryCode_non_normalized>
// <Address_indicator_non_normalized/>
// <Residence_code_non_normalized/>
// </CAIS_Holder_Address_Details>
// <CAIS_Holder_Phone_Details>
// <Telephone_Number>9819137672</Telephone_Number>
// <Telephone_Type/>
// <Telephone_Extension/>
// <Mobile_Telephone_Number/>
// <FaxNumber/>
// <EMailId/>
// </CAIS_Holder_Phone_Details>
// <CAIS_Holder_ID_Details>
// <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
// <PAN_Issue_Date/>
// <PAN_Expiration_Date/>
// <Passport_Number/>
// <Passport_Issue_Date/>
// <Passport_Expiration_Date/>
// <Voter_ID_Number/>
// <Voter_ID_Issue_Date/>
// <Voter_ID_Expiration_Date/>
// <Driver_License_Number/>
// <Driver_License_Issue_Date/>
// <Driver_License_Expiration_Date/>
// <Ration_Card_Number/>
// <Ration_Card_Issue_Date/>
// <Ration_Card_Expiration_Date/>
// <Universal_ID_Number/>
// <Universal_ID_Issue_Date/>
// <Universal_ID_Expiration_Date/>
// <EMailId/>
// </CAIS_Holder_ID_Details>
// </CAIS_Account_DETAILS>
// <CAIS_Account_DETAILS>
// <Identification_Number/>
// <Subscriber_Name> icicibank</Subscriber_Name>
// <Account_Number>ICICI8131307</Account_Number>
// <Portfolio_Type>I</Portfolio_Type>
// <Account_Type>51</Account_Type>
// <Open_Date>19940717</Open_Date>
// <Credit_Limit_Amount/>
// <Highest_Credit_or_Original_Loan_Amount>50000</Highest_Credit_or_Original_Loan_Amount>
// <Terms_Duration/>
// <Terms_Frequency/>
// <Scheduled_Monthly_Payment_Amount/>
// <Account_Status>71</Account_Status>
// <Payment_Rating>1</Payment_Rating>
// <Payment_History_Profile>11110000000000001???????????????????</Payment_History_Profile>
// <Special_Comment/>
// <Current_Balance>85000</Current_Balance>
// <Amount_Past_Due>1200</Amount_Past_Due>
// <Original_Charge_Off_Amount/>
// <Date_Reported>20190130</Date_Reported>
// <Date_of_First_Delinquency/>
// <Date_Closed/>
// <Date_of_Last_Payment/>
// <SuitFiledWillfulDefaultWrittenOffStatus/>
// <SuitFiled_WilfulDefault/>
// <Written_off_Settled_Status/>
// <Value_of_Credits_Last_Month/>
// <Occupation_Code/>
// <Settlement_Amount/>
// <Value_of_Collateral/>
// <Type_of_Collateral/>
// <Written_Off_Amt_Total/>
// <Written_Off_Amt_Principal/>
// <Rate_of_Interest/>
// <Repayment_Tenure>0</Repayment_Tenure>
// <Promotional_Rate_Flag/>
// <Income/>
// <Income_Indicator/>
// <Income_Frequency_Indicator/>
// <DefaultStatusDate/>
// <LitigationStatusDate/>
// <WriteOffStatusDate/>
// <DateOfAddition>20170830</DateOfAddition>
// <CurrencyCode>INR</CurrencyCode>
// <Subscriber_comments/>
// <Consumer_comments/>
// <AccountHoldertypeCode>1</AccountHoldertypeCode>
// <CAIS_Account_History>
// <Year>2019</Year>
// <Month>01</Month>
// <Days_Past_Due>38</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>12</Month>
// <Days_Past_Due>38</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>11</Month>
// <Days_Past_Due>36</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>10</Month>
// <Days_Past_Due>34</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>09</Month>
// <Days_Past_Due>32</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>08</Month>
// <Days_Past_Due>24</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>07</Month>
// <Days_Past_Due>23</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>06</Month>
// <Days_Past_Due>22</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>05</Month>
// <Days_Past_Due>21</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>04</Month>
// <Days_Past_Due>20</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>03</Month>
// <Days_Past_Due>19</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>02</Month>
// <Days_Past_Due>18</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2018</Year>
// <Month>01</Month>
// <Days_Past_Due>17</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>12</Month>
// <Days_Past_Due>16</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>11</Month>
// <Days_Past_Due>15</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>10</Month>
// <Days_Past_Due>14</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>09</Month>
// <Days_Past_Due>13</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Account_History>
// <Year>2017</Year>
// <Month>08</Month>
// <Days_Past_Due>40</Days_Past_Due>
// <Asset_Classification>?</Asset_Classification>
// </CAIS_Account_History>
// <CAIS_Holder_Details>
// <Surname_Non_Normalized>SAGAR</Surname_Non_Normalized>
// <First_Name_Non_Normalized>UGLE</First_Name_Non_Normalized>
// <Middle_Name_1_Non_Normalized/>
// <Middle_Name_2_Non_Normalized/>
// <Middle_Name_3_Non_Normalized/>
// <Alias/>
// <Gender_Code>1</Gender_Code>
// <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
// <Passport_Number/>
// <Voter_ID_Number/>
// <Date_of_birth>19820322</Date_of_birth>
// </CAIS_Holder_Details>
// <CAIS_Holder_Address_Details>
// <First_Line_Of_Address_non_normalized>J-1 110</First_Line_Of_Address_non_normalized>
// <Second_Line_Of_Address_non_normalized>ANANDI VIHAR</Second_Line_Of_Address_non_normalized>
// <Third_Line_Of_Address_non_normalized>DEHU ROAD</Third_Line_Of_Address_non_normalized>
// <City_non_normalized>DIGHI</City_non_normalized>
// <Fifth_Line_Of_Address_non_normalized/>
// <State_non_normalized>27</State_non_normalized>
// <ZIP_Postal_Code_non_normalized>412216</ZIP_Postal_Code_non_normalized>
// <CountryCode_non_normalized>IB</CountryCode_non_normalized>
// <Address_indicator_non_normalized/>
// <Residence_code_non_normalized/>
// </CAIS_Holder_Address_Details>
// <CAIS_Holder_Phone_Details>
// <Telephone_Number>9819137672</Telephone_Number>
// <Telephone_Type/>
// <Telephone_Extension/>
// <Mobile_Telephone_Number/>
// <FaxNumber/>
// <EMailId/>
// </CAIS_Holder_Phone_Details>
// <CAIS_Holder_ID_Details>
// <Income_TAX_PAN>AOZPB0247S</Income_TAX_PAN>
// <PAN_Issue_Date/>
// <PAN_Expiration_Date/>
// <Passport_Number/>
// <Passport_Issue_Date/>
// <Passport_Expiration_Date/>
// <Voter_ID_Number/>
// <Voter_ID_Issue_Date/>
// <Voter_ID_Expiration_Date/>
// <Driver_License_Number/>
// <Driver_License_Issue_Date/>
// <Driver_License_Expiration_Date/>
// <Ration_Card_Number/>
// <Ration_Card_Issue_Date/>
// <Ration_Card_Expiration_Date/>
// <Universal_ID_Number/>
// <Universal_ID_Issue_Date/>
// <Universal_ID_Expiration_Date/>
// <EMailId/>
// </CAIS_Holder_ID_Details>
// </CAIS_Account_DETAILS>
// </CAIS_Account>
// <Match_result>
// <Exact_match>Y</Exact_match>
// </Match_result>
// <TotalCAPS_Summary>
// <TotalCAPSLast7Days>0</TotalCAPSLast7Days>
// <TotalCAPSLast30Days>0</TotalCAPSLast30Days>
// <TotalCAPSLast90Days>0</TotalCAPSLast90Days>
// <TotalCAPSLast180Days>0</TotalCAPSLast180Days>
// </TotalCAPS_Summary>
// <CAPS>
// <CAPS_Summary>
// <CAPSLast7Days>0</CAPSLast7Days>
// <CAPSLast30Days>0</CAPSLast30Days>
// <CAPSLast90Days>0</CAPSLast90Days>
// <CAPSLast180Days>0</CAPSLast180Days>
// </CAPS_Summary>
// </CAPS>
// <NonCreditCAPS>
// <NonCreditCAPS_Summary>
// <NonCreditCAPSLast7Days>0</NonCreditCAPSLast7Days>
// <NonCreditCAPSLast30Days>0</NonCreditCAPSLast30Days>
// <NonCreditCAPSLast90Days>0</NonCreditCAPSLast90Days>
// <NonCreditCAPSLast180Days>0</NonCreditCAPSLast180Days>
// </NonCreditCAPS_Summary>
// </NonCreditCAPS>
// <SCORE>
// <BureauScore>719</BureauScore>
// <BureauScoreConfidLevel>H</BureauScoreConfidLevel>
// </SCORE>
// </INProfileResponse>
// `

// parseXml(xml)
//     .then((res) => console.log(res.creditAccountsInfo))
//     .catch((err) => console.log(err));


module.exports = { parseXml };
